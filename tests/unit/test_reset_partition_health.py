...
def test_reset_partition_health_with_location_and_email() -> None:
    ...
    try:
        c.partition_health_table_name = "catalog.schema.partition_health"
        ...
        with patch.object(STL, "sql_and_log") as mock_stl:
            ...
            assert drop_call[0][0] == "drop table if exists catalog.schema.partition_health"

            # Check CREATE statement
            create_call: Any = mock_stl.call_args_list[1]
            create_sql: str = create_call[0][0]
            # Validate SQL structure
            assert "create table catalog.schema.partition_health" in create_sql
            assert "using iceberg" in create_sql
            assert "partitioned by (days(start_time), catalog, schema)" in create_sql
            assert "location 'file:///iceberg/admin/partition_health'" in create_sql
...
def test_reset_partition_health_with_no_location_and_no_email() -> None:
    # Reload config to pick up template override
    c = Config.instance()
    prev_maintenance_schedule_table_name = c.maintenance_schedule_table_name
    prev_admin_table_location = c.admin_table_location
    ...
    try:
        ...
        with patch.object(STL, "sql_and_log") as mock_stl:
            ...
            assert "using iceberg" in create_sql
            assert "partitioned by (days(start_time), catalog, schema)" in create_sql
            assert "location 'file:///iceberg/admin/partition_health'" not in create_sql
