...
| ice-keeper.widening.rule.min-partition-to-widen     | 1d            | Minimum partition time offset (relative to the most recent partition) to start widening. Uses the same duration format as `ice-keeper.min-partition-to-optimize` (e.g., `1d`, `0m`, `2m`).
| ice-keeper.widening.rule.max-partition-to-widen     | 7d            | Maximum partition time offset (relative to the most recent partition) to consider for widening. Uses the same duration format as `ice-keeper.min-partition-to-optimize` (e.g., `7d`, `12m`).
