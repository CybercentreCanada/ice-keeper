...
| ice-keeper.min-age-to-optimize                 | 2                  | Number of recent partitions to ignore by the optimization. For example you might want to ignore the current hour or the current day. The current age is given index of 1, by setting `ice-keeper.min-age-to-optimize=2` you can skip the current hour or day.
| ice-keeper.max-age-to-optimize                 | 72                 | Number of partitions to consider optimizing. Note this will be the number of months, days or hours depending on the base time partition.
