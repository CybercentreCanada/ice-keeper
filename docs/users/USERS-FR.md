...
| ice-keeper.min-partition-to-optimize            | 1d                 | Décalage d'âge minimum des partitions à inclure dans l'optimisation. Format : `<entier><unité>` où l'unité est `h` (heure) ou `d` (jour). `min` et `max` doivent utiliser la même unité. |
| ice-keeper.max-partition-to-optimize            | 7d                 | Décalage d'âge maximum des partitions à inclure dans l'optimisation, avec le même format et la même unité que `ice-keeper.min-partition-to-optimize`. La fenêtre est inclusive aux deux bornes (`>= current - max` et `<= current - min`). Doit être supérieur ou égal à `min`. Pour les tables partitionnées par `year(...)`, cette fenêtre d'optimisation n'est pas appliquée (`1=1`). |
