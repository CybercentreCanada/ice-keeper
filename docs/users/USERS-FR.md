...
| ice-keeper.max-partition-to-optimize            | 7d                 | Décalage temporel maximum de la partition (par rapport à la partition la plus récente) à considérer pour l'optimisation. Utilise le même format de durée que `ice-keeper.min-partition-to-optimize`. |
...
| ice-keeper.widening.rule.min-partition-to-widen | 1d                 | Décalage temporel minimum de la partition (par rapport à la partition la plus récente) pour commencer l'élargissement. Utilise le même format de durée que `ice-keeper.min-partition-to-optimize` (ex : `1d`, `0m`, `2m`). |
| ice-keeper.widening.rule.max-partition-to-widen | 7d                 | Décalage temporel maximum de la partition (par rapport à la partition la plus récente) à considérer pour l'élargissement. Utilise le même format de durée que `ice-keeper.min-partition-to-optimize` (ex : `7d`, `12m`). |
