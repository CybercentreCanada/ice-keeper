...
- **Expiration des instantanés** — Activée par défaut. ice-keeper expire les anciens instantanés pour garder les métadonnées compactes et récupérer de l'espace de stockage. Vous contrôlez la fenêtre de rétention (en jours) et le nombre minimum d'instantanés à conserver.
    ...
    - `expire` : expiration via la procédure Iceberg (peut aussi supprimer des fichiers non suivis)
    - `expire_fast` : nettoyage rapide des instantanés via PyIceberg, uniquement sur les métadonnées (ne supprime pas les fichiers non suivis)
    Un modèle courant est d'exécuter `expire_fast` chaque jour, puis `orphan` une fois par semaine.
...
| ice-keeper.min-partition-to-optimize            | 1d                 | Décalage d'âge minimum des partitions à inclure dans l'optimisation. Format : `<entier><unité>` où l'unité est `h` (heure), `d` (jour), `m` (mois) ou `y` (année), insensible à la casse (par exemple, `1d`, `1D`, `3M`, `3m` fonctionnent tous). `min` et `max` doivent utiliser la même unité. |
| ice-keeper.max-partition-to-optimize            | 7d                 | Décalage d'âge maximum des partitions à inclure dans l'optimisation, avec le même format et la même unité que `ice-keeper.min-partition-to-optimize`. La fenêtre est inclusive aux deux bornes (`>= current - max` et `<= current - min`). Doit être supérieur ou égal à `min`. |
| ice-keeper.optimization-strategy               | Aucun              | Définit la stratégie d'optimisation à utiliser. |
| ice-keeper.optimize-partition-depth            | -1 (regroupement dynamique) | Contrôle le nombre de niveaux de partition utilisés pour regrouper les partitions lors de l'optimisation. La valeur par défaut de `-1` active le regroupement dynamique, qui regroupe automatiquement les sous-partitions en lots jusqu'à `ice-keeper.optimization-grouping-size-bytes` pour un seul appel à `rewrite_data_files`. À **depth=1**, ice-keeper analyse les sous-partitions individuellement mais regroupe l'optimisation par le premier niveau uniquement. À **depth=2**, chaque sous-partition est optimisée indépendamment. Doit être `-1` ou un entier positif (1, 2, 3, ...) ; toute autre valeur (ex : 0) génère une erreur. |
