...
class WideningRule:
    ...
    def make_widening_validation_filter(self, partition_diagnosiss: PartitionDiagnosisResult) -> str:
        ...
        if len(partition_diagnosiss.partition_filters) != 1:
            msg = (
                "Widening validation requires exactly one partition filter, "
                f"but got {len(partition_diagnosiss.partition_filters)}. "
                "Multiple partition filters are not supported by this widening rule."
            )
            raise ValueError(msg)
        partition_filter = partition_diagnosiss.partition_filters[0]
